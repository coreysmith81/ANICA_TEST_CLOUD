Report 50057 "Rebate Distribution Report"
{
        ...
        dataitem("Rebates Customers"; "Rebates Customers")
        {
            ...
            dataitem("Rebates Detail"; "Rebates Detail")
            {
                ...
                begin
                    ...
                    if DetailCounter = NoRebateDetailRecs then begin
                        if CustRebate <> 0 then begin
                            //Calc Minimums
                            if CustRebate > MinRebate then begin
                                if VProcess = true then begin
                                    CreateVendor;
                                    CreateVendorCheckEntry;
                                end
                            end
                            else
                                CustMinRebate := CustRebate;
                        end;
                        //Get GrandTotals
            ...
            begin
                ...
                if "Rebates Customers"."Rebate Customer No." <> LastCustomerNo then begin
                    CustMinRebate := 0;
                    if CustRebate <> 0 then begin
                        //Calc Minimums
                        if CustRebate > MinRebate then begin
    ...
    begin
        ...
        if RebateDetail.Find('-') then begin
            repeat
    ...
    begin
        ...
        if RebateCustomers.Find('+') then begin
    ...
    begin
        ...
        GenJnlLine1."Journal Batch Name" := 'REBATES';
        GetLineNumber;
        GenJnlLine1."Shortcut Dimension 1 Code" := 'ANICA';
        GenJnlLine1.Insert(true);
        GenJnlLine1."Posting Date" := Today;
        GenJnlLine1.Validate("Posting Date");
        GenJnlLine1."Document Date" := Today;
        GenJnlLine1.Validate("Document Date");
        GenJnlLine1."Document Type" := "Gen. Journal Document Type"::Invoice;
        ...
        GenJnlLine1.Validate("External Document No.");
        GenJnlLine1."Account Type" := "Gen. Journal Account Type"::Vendor;
        GenJnlLine1.Validate("Account Type");
        CreateVendor;
        GenJnlLine1."Account No." := VendorNumber;
        GenJnlLine1.Validate("Account No.");
        GenJnlLine1.Validate(Description);
        GenJnlLine1.Amount := CustRebate * -1;
        GenJnlLine1.Validate(Amount);
        GenJnlLine1.Validate("Shortcut Dimension 1 Code");
        GenJnlLine1."Bal. Account Type" := "Gen. Journal Account Type"::"G/L Account";
        GenJnlLine1.Validate("Bal. Account Type");
        GenJnlLine1."Bal. Account No." := '144-01';
        GenJnlLine1.Validate("Bal. Account No.");
        if VDueDate <> 0D then
            GenJnlLine1."Due Date" := VDueDate
        else
            GenJnlLine1."Due Date" := Today;
