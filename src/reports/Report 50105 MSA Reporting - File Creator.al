Report 50105 "MSA Reporting - File Creator"
{
    ...
    begin
        ...
        VHIDoutput := 'HID' +
        HID."Distributor ID Number" +
        PadStr('TOB', 4, ' ') +
        PadStr(VTestLiveFlag, 1, ' ') +
        'W' +
        VTextEndDate +
        PadStr(HID."Distributor Name", 32, ' ') +
        PadStr(HID."Distributor Address", 90, ' ') +
        PadStr(HID."Distributor City", 25, ' ') +
        PadStr(HID."Distributor State", 2, ' ') +
        PadStr(HID."Distributor Zip Code", 9, ' ') +
        'USA' + PadStr(HID."Distributor Contact First Name", 20, ' ') +
        PadStr(HID."Distributor Contact Last Name", 20, ' ') +
        PadStr(HID."Country/City Dialing Code (T)", 5, ' ') +
        PadStr(HID."Distributor Contact Phone No.", 10, ' ') +
        PadStr(HID."Country/City Dialing Code (F)", 5, ' ') +
        PadStr(HID."Distributor Contact Fax No.", 10, ' ') +
        PadStr(HID."Distributor Contact Email", 60, ' ') +
        PadStr('0001', 4, ' ') +
        PadStr('0000', 4, ' ') +
        PadStr('0001', 4, ' ') +
    ...
    begin
        ...
        if Item.Find('-') then
            ...
            begin
                ...
                case Item."Component Shipper Flag" of
                    0:
                        VComponentShipperFlag := ' ';
                    1:
                        VComponentShipperFlag := 'S';
                    2:
                        VComponentShipperFlag := 'C';
                end;

                VAltUPC1 := Item."Alternate UPC Code 1";
                ...
                VBIDoutput := 'BID' +
                    PadStr(VUPC, 14, ' ') +
                    PadStr(VItemNo, 14, ' ') +
                    PadStr(Item."MSA Item Description", 50, ' ') +
                    PadStr(Item."Promotion Description", 50, ' ') +
                    PadStr(VItemsPer, 6, ' ') +
                    PadStr(VPromoIndicatorText, 1, ' ') +
                    PadStr(Item."NACS Standard Category Code", 6, ' ') +
                    PadStr(Item."MSA Category Code", 6, ' ') +
                    '          ' +
                    PadStr(VPackText, 6, ' ') +
                    PadStr(VPackDescription, 10, ' ') +
                    PadStr(VComponentShipperFlag, 1, ' ') +
                    PadStr(Item."Manufacturer Promo Code", 10, ' ') +
                    PadStr(Item."Manufacturer Prod. ID Code", 14, ' ') +
                    PadStr(Item."UPC Extension", 2, ' ') +
                    PadStr(Item."UPC year/issue Extension", 4, ' ') +
                    PadStr('  ', 2, ' ') +
                    PadStr(VAltUPC1, 16, ' ') +
                    PadStr(VAltUPC2, 16, ' ') +
        ...
        if SIDs.Find('-') then begin
            ...
            begin
                ...
                if Customer.Find('-') then begin
                    ...
                    case Customer."Ship-To Cust. Class of Trade" of
                        0:
                            VClassOfTrade := 'Retailer';
                        1:
                            VClassOfTrade := 'Distributor';
                        2:
                            VClassOfTrade := 'Other';
                    end;

                    case Customer."Ship-To Cash and Carry" of
                        true:
                            VCashCarry := 'Y';
                        false:
                            VCashCarry := 'N';
                    ...
                    case Customer."Cust. Prod. Promo Acceptance" of
                        true:
                            VPromoAccept := 'Y';
                        false:
                            VPromoAccept := 'N';
                    ...
                    VSIDoutput := 'SID' +
                    PadStr(Customer."No.", 8, ' ') +
                    PadStr(Customer."Telxon Store number", 8, ' ') +
                    PadStr(Customer."Ship-To Cust. Shipping No. Ext", 8, ' ') +
                    PadStr(Customer.Name, 32, ' ') +
                    PadStr(Customer."Telxon Store number", 8, ' ') +
                    PadStr(Customer."Store Physical Address", 90, ' ') +
                    PadStr(Customer.City, 25, ' ') +
                    PadStr(Customer.County, 2, ' ') +
                    PadStr(Customer."Post Code", 9, ' ') +
                    'USA' +
                    PadStr('AK', 2, ' ') +
                    '   ' +
                    PadStr('', 10, ' ') +
                    PadStr(VClassOfTrade, 20, ' ') +
                    PadStr(Customer."Ship-To Cust. TDLinx No.", 7, ' ') +
                    PadStr(VCashCarry, 1, ' ') +
                    PadStr(Customer."Location No.", 9, ' ') +
                    PadStr(Customer."Machine Type", 9, ' ') +
                    ' ' +
                    PadStr(Customer."No.", 24, ' ') +
                    PadStr(Customer."Bill-To Cust. Group No.", 24, ' ') +
                    PadStr(Customer.Name, 32, ' ') +
                    PadStr(Customer."Store Physical Address", 90, ' ') +
                    PadStr(Customer.City, 25, ' ') +
                    PadStr(Customer.County, 2, ' ') +
                    ...
                    PadStr(Customer."Sales Rep ID No.", 10, ' ') +
                    PadStr('', 29, ' ') +
                    PadStr('', 3, ' ') +
    ...
    begin
        ...
        if PUR.Find('-') then
            repeat
            begin
                VAmount := Format(PUR.Amount, 0, 1);
                while StrLen(VAmount) < 11 do
                    VAmount := '0' + VAmount;

                VQuantity := Format(PUR.Quantity, 0, 1);
                while StrLen(VQuantity) < 11 do
                    VQuantity := '0' + VQuantity;

                VPURItemNo := PUR."Item No.";
                ...
                PadStr(PUR."Customer Shipping No.", 8, ' ') +
                PadStr(PUR."Customer Shipping No. Ext.", 8, ' ') +
                VPURItemNo +
                //PADSTR(VItemNo,14,' ') +
                //PADSTR("Item No.",14,' ') +
                PadStr('', 3, ' ') +
                PadStr(PUR."Invoice No.", 30, ' ') +
                PadStr(PUR.Date, 8, ' ') +
                PadStr('', 20, ' ') +
                '001' +
                PadStr(VQuantity, 11, ' ') +
    ...
    begin
        if HID.Find('-') then begin
            ...
            VTOToutput := 'TOT' +
            PadStr(HID."Distributor ID Number", 8, ' ') +
