Report 50000 "Freight Calculation"
{
    ...
    begin
        ...
        //>>MAS, 06/20/01, MAS200
        if Item."Commodity Code" = 'OS' then
            begin
        ...
        Item.Get(SalesLine."No.");
        if Item."Gen. Prod. Posting Group" = 'STOREU' then
            begin
                SalesLine."Retail Price Per Unit" := 0;
                exit;
            end;
                ...
                if CustomerTarget.Find('-') then
                    begin
                        repeat
                            ...
                            if (VCurrPriority > VHighPriority) and (VCurrRetail <> 0) then
                                begin
    ...
    begin
        if not ParcPostDirRate.Get(Item."Commodity Code",ShipLeg."Sea or Anch",ShipLeg."Priority Type") then
          begin
    ...
    begin
        Customer.Get(SalesLine."Sell-to Customer No.");
        if not BargeRate.Get(Customer."Barge Zone",Item."Freight Code",ShipLeg.Carrier) then
          begin
    ...
    begin
        Customer.Get(SalesLine."Sell-to Customer No.");
        if not WarehParcPostRate.Get(Customer."Parcel Post Code",ShipLeg."Priority Type") then
          begin
    ...
    var
        ...
        _EstimatedFreight: Decimal;
    begin
        if not SteamShipTruckRate.Get(Item."Freight Code",ShipLeg.Carrier,ShipLeg.Origin,ShipLeg.Destination) then
          begin
    ...
    begin
        ...
        if not Commodity.Get(Item."Commodity Code") then
          begin
    ...
    begin
        ...
        if SalesLine.Find('-') then repeat
            ...
            else if (Item."Tobacco Type" = 2) and (Item."No." <> '0001-0078') then
