Report 50109 "JBG Weekly Reused Prices"
{
    DefaultLayout = RDLC;
    RDLCLayout = './Layouts/JBG Weekly Reused Prices.rdlc';
    ...
    dataset
    {
        dataitem("JBG Price Weekly Table";"JBG Price Weekly Table")
        {
            ...
            begin
                ...
                if (VIsFound = true) and (VCheckUPC <> VNewUPC) and (VNewGrossWt <> VCheckGrossWt) then
                    begin
                        ...
                        CheckNonStockItem;
                        if VNonStockEntryNo = '' then
                            SecondaryNSEntryNoLookup; //CS 5-14-13: Added to find NS Entry of items with no previous entry.

                        if (VIsFound <> true) and (VNoPrevRecord <> true) then //CS: Added VNoPrevRecord clause
                            CurrReport.Skip
                        else
                            begin
                                ...
                                if VIsFound <> true then
                                    ...
                                    with "JBG Price Weekly Table" do begin
                                        VCapCode := "Cap Code";
                        ...
                        //========================================================================
                        //START OF WHAT WAS COPIED OVER FROM OLD DATAPORT.
                        //========================================================================

                        //The price import file must be sorted by item, by Cap Code

                        //Get Nonstock Entry Number
                        VNSEntryNoLookup := VNonStockEntryNo;
                        VNonStockItemLookup := VJBGItemNo;

                        //Determine if the Zone Record should be created from the previous records read
                        //NOTE: Changes here need to be added to the Post Data Item Section for the last item read
                        //Create Nonstock Target Retails
                        //CS 11-08-16: Updated with Target Retail table
                        LookupNonStockItem;
                        CreateNonstockTargetRetail;

                        //If a stock item exists, create Target retail records for it also
                        ...
                        //========================================================================
                        //END OF WHAT WAS COPIED OVER FROM OLD DATAPORT.
                        //========================================================================
    ...
    begin
        ...
        if JBGItemTable.Find('+') then
            with JBGItemTable do begin
                VNewUPC := UPC;
                VDescr := Description;
                VNewPack := Pack;
                VNewPackDescr := "Pack Descrip";
                VNewGrossWt := "Gross Weight";
                VIsFound := true;
            end
    ...
    begin
        //Create or Update Nonstock Target Retail.
