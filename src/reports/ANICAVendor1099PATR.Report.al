Report 50189 "ANICA Vendor 1099 PATR"
{
    ...
    begin
        ...
        begin
            ...
            TempAppliedEntry.SetRange("IRS 1099 Code", 'PATR-', 'PATR-99');
            if TempAppliedEntry.Find('-') then
                repeat
                    Calculate1099Amount(TempAppliedEntry, PaymentEntry, TempAppliedEntry."Amount to Apply");
    ...
    begin
        if TestPrint then begin
            ...
            CompanyAddress[1] := UpperCase(CompanyInfo.Name);
            CompanyAddress[2] := UpperCase(CompanyInfo.Address);
            CompanyAddress[3] := UpperCase(CompanyInfo."Address 2");
            if StrLen(CompanyInfo.City + ', ' + CompanyInfo.County + '  ' + CompanyInfo."Post Code") > MaxStrLen(CompanyAddress[4]) then begin
                CompanyAddress[4] := UpperCase(CompanyInfo.City);
                CompanyAddress[5] := UpperCase(CompanyInfo.County) + '  ' + CompanyInfo."Post Code";
                ...
                if (CompanyInfo.City <> '') and (CompanyInfo.County <> '') then begin
                    CompanyAddress[4] := UpperCase(CompanyInfo.City) + ', ' + UpperCase(CompanyInfo.County) + '  ' + CompanyInfo."Post Code";
                    CompanyAddress[5] := CompanyInfo."Phone No.";
                end else begin
